module FileUtils
  ...
  METHODS = ["remove_file", "copy_stream", "rmdir", "safe_unlink", "compare_stream", "ln", "install", "mkpath", "identical?", "remove", "chown_R", "chmod", "cp", "mv", "getwd", "rmtree", "move", "remove_dir", "symlink", "cp_r", "ln_s", "rm_r", "cmp", "copy_file", "remove_entry_secure", "mkdir", "chmod_R", "makedirs", "copy_entry", "rm_f", "chown", "pwd", "uptodate?", "rm", "compare_file", "mkdir_p", "ln_sf", "cd", "chdir", "rm_rf", "touch", "link", "copy", "remove_entry"]
  NoWrite = FileUtils::NoWrite
  OPT_TABLE = {"touch"=>[:noop, :verbose, :mtime, :nocreate], "mv"=>[:force, :noop, :verbose, :secure], "chmod_R"=>[:noop, :verbose, :force], "rm_r"=>[:force, :noop, :verbose, :secure], "rm_f"=>[:noop, :verbose], "cp"=>[:preserve, :noop, :verbose], "symlink"=>[:force, :noop, :verbose], "mkpath"=>[:mode, :noop, :verbose], "mkdir"=>[:mode, :noop, :verbose], "cd"=>[:verbose], "remove"=>[:force, :noop, :verbose], "ln_s"=>[:force, :noop, :verbose], "link"=>[:force, :noop, :verbose], "chdir"=>[:verbose], "safe_unlink"=>[:noop, :verbose], "cp_r"=>[:preserve, :noop, :verbose, :dereference_root, :remove_destination], "chown"=>[:noop, :verbose], "rm"=>[:force, :noop, :verbose], "makedirs"=>[:mode, :noop, :verbose], "ln_sf"=>[:noop, :verbose], "mkdir_p"=>[:mode, :noop, :verbose], "move"=>[:force, :noop, :verbose, :secure], "rmdir"=>[:noop, :verbose], "copy"=>[:preserve, :noop, :verbose], "rm_rf"=>[:noop, :verbose, :secure], "rmtree"=>[:noop, :verbose, :secure], "chmod"=>[:noop, :verbose], "install"=>[:mode, :preserve, :noop, :verbose], "chown_R"=>[:noop, :verbose, :force], "ln"=>[:force, :noop, :verbose]}
